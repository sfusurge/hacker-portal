...
export const HOURLY_QUOTA = Number.isFinite(rawHourly)
    ? Math.max(0, rawHourly)
    : 100;
...
export interface ProcessEmailQueueOptions {
    // Only process rows for this hackathon.
    hackathonId?: number;
    // Only process these specific queue rows (for per-row "Send now").
    ids?: number[];
    // When false, ignore the hourly quota (admin emergency override).
...
export interface ProcessEmailQueueResult {
    ...
    hourlyQuota: number;
    sentInLastHourBeforeRun: number;
    remainingHourlyQuotaAfterRun: number;
    errors?: string[];
    message?: string;
}

/**
 * Drains pending rows from the email queue and sends them over SMTP. Shared by
 * the cron (`/api/cron/process-emails`) and the admin "Process queue now" /
 * "Send now" mutations. Keeps the exact retry + hourly-quota + SMTP-throttle
 * semantics the cron has always used.
 */
export async function processEmailQueue(
    opts: ProcessEmailQueueOptions = {}
): Promise<ProcessEmailQueueResult> {
    const respectQuota = opts.respectQuota ?? true;

    const conditions: SQL[] = [
        eq(emailQueue.status, 'pending'),
        lt(emailQueue.failedCount, MAX_RETRIES),
    ];
    if (opts.hackathonId != null) {
        conditions.push(eq(emailQueue.hackathonId, opts.hackathonId));
    }
    if (opts.ids && opts.ids.length > 0) {
        conditions.push(inArray(emailQueue.id, opts.ids));
    }
    const pendingPredicate = and(...conditions);

    const empty = (
        message: string,
        extra: Partial<ProcessEmailQueueResult> = {}
    ) => ({
        success: true,
        sent: 0,
        failed: 0,
        ...
        sentInLastHourBeforeRun: 0,
        remainingHourlyQuotaAfterRun: 0,
        message,
        ...extra,
    });

    if (respectQuota && HOURLY_QUOTA === 0) {
        return empty('EMAIL_HOURLY_QUOTA is zero (sending disabled)');
    }
    ...
    const [sentCountResult] = await databaseClient
        ...
        .from(emailQueue)
        .where(
            and(eq(emailQueue.status, 'sent'), gte(emailQueue.sentAt, hourAgo))
        );

    const sentInHour = Number(sentCountResult?.count ?? 0);
    const remainingHourlyQuota = Math.max(0, HOURLY_QUOTA - sentInHour);

    if (respectQuota && remainingHourlyQuota === 0) {
        return empty('Hourly quota reached', {
    ...
    if (pendingEmails.length === 0) {
        return empty('No pending emails', {
            sentInLastHourBeforeRun: sentInHour,
            remainingHourlyQuotaAfterRun: remainingHourlyQuota,
        });
    }

    const templateIds = [...new Set(pendingEmails.map((e) => e.templateId))];
    ...
    for (const pendingEmail of pendingEmails) {
        try {
            ...
            const templateData = {
                firstName:
                    userData?.firstName ?? pendingEmail.firstName ?? 'Friend',
            ...
            await databaseClient
                ...
                .set({
                    status:
                        newFailedCount >= MAX_RETRIES ? 'failed' : 'pending',
