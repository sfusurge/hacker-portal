...
import { insertHackathonSchema, hackathons, deleteHackathonSchema} from "@/db/schema/hackathons";
...
export const hackathonsRouter = router({
    ...
    deleteHackathon: publicProcedure.input(deleteHackathonSchema).mutation(async (opts) => {
        await databaseClient.delete(hackathons).where(eq(hackathons.hackathon_id, opts.input.id));
    }),
});
